...
void add_tree_node(AVLNode *new_node, int (*compare)(AVLNode *, AVLNode *)) {
    AVLNode **find = find_tree_node(&root, new_node, compare);
    if((*find) == NULL) {
        *find = new_node;
        root->height = 1 + fmax(node_height(root->left), node_height(root->right));
...
// The problem : node_detach returns the new pointer
// But we also want it to return the deleted node, so that we can free it
AVLNode* node_detach(AVLNode **p_node) {
    AVLNode *node = *p_node;
    assert(node != NULL);
...
// TODO : maybe change the api to expect delete from root ?
AVLNode* remove_tree_node(AVLNode *node, AVLNode *key, int (*compare)(AVLNode *, AVLNode *)) {
    // if(node == NULL) return NULL;

    AVLNode **find = find_tree_node(&node, key, compare);
    if(*find == NULL) return root;

    AVLNode* result =  node_detach(find);
    root = avl_rebalance(root);
    return result;

    // int r = compare(node, key);
    // if(r < 0){
    //     node->left = remove_tree_node(node->left, key, compare);
    //     return node;
    // }
    // else if(r > 0){
    //     node->right = remove_tree_node(node->right, key, compare);
    //     return node;
    // }
    // AVLNode* result =  node_detach(node);
    // root = avl_rebalance(root);
    // return result;
}
...
void display_tree(int(cb)(AVLNode *)) { print_node(root, 0, cb); printf("=====\n"); }
...
static void update_height(AVLNode *node) {
    node->height = fmax(node_height(node->left), node_height(node->right));
}
